...
• Passo “Escolher tipo” usa AddUserFlow.CHOOSING_ROLE.
• Eliminada dependência de AddUserStates.
• Restante lógica (Voltar, fecho de menus, etc.) mantém-se.
"""
...
from bot.states.add_user_flow     import AddUserFlow          # ⟵ novo import
from bot.menus.common             import back_button, start_menu_timeout, cancel_back_kbd
...
# ───────────────────────── utilitários UI ─────────────────────────
async def _replace_menu(cb: types.CallbackQuery, state: FSMContext, text: str,
                        kbd: types.InlineKeyboardMarkup) -> None:
    """Edita (ou cria) a mensagem-menu e reinicia timeout."""
...
async def _add_user(cb: types.CallbackQuery, state: FSMContext):
    await state.set_state(AddUserFlow.CHOOSING_ROLE)          # ⟵ usa nova FSM
    await _replace_menu(
        cb, state,
...
async def adduser_choose_role(cb: types.CallbackQuery, state: FSMContext):
    ...
    # guarda e inicia questionário detalhado
