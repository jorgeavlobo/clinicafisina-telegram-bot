...
from bot.menus import show_menu                    # para enviar o menu
...
def confirm_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[[  # linha única com dois botões
...
async def handle_contact(message: Message, state: FSMContext) -> None:
    ...
    Nota: agora guardamos menu_msg_id/menu_chat_id - requerido
    pelo ActiveMenuMiddleware para considerar o menu activo.
    """
    ...
    if user:
        ...
        # 🔑  REGISTAR menu activo  — necessário para ActiveMenuMiddleware
        await state.update_data(
            menu_msg_id  = confirm_msg.message_id,
            menu_chat_id = confirm_msg.chat.id,
        )
    else:
        await state.clear()
        await message.answer(
...
async def confirm_link(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    await show_menu(cb.bot, cb.message.chat.id, state, roles)   # mostra menu
